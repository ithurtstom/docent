...
struct ConnectiveModelState : public FeatureFunction::State, public FeatureFunction::StateModifications {
	...
	Float matchConnectiveDictionary(const AnchoredPhrasePair& pair) {
		...
		for (uint j = 0; j < pair.second.get().getSourcePhrase().get().size(); j++) {
			 if (boost::regex_match(pair.second.get().getSourcePhrase().get()[j], resultsSrc, regSource)) {
				 	...
				 	if (!pair.second.get().getTargetPhrase().get()[j].empty()) {
				    if (!boost::regex_match(pair.second.get().getTargetPhrase().get()[j], resultsTrg, regTarget)) {
				 		 fin++;
				 		 LOG(logger_, debug, "Increase for: " << pair.second.get().getSourcePhrase().get()[j] << " -- " << pair.second.get().getTargetPhrase().get()[j]);
				 	} else {
				 	     fin--;
				 	     LOG(logger_, debug, "Decrease for: " << pair.second.get().getSourcePhrase().get()[j] << " -- deleting: " << pair.second.get().getTargetPhrase().get()[j]);
